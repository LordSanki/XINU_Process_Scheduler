...
static int createProc(procaddr,ssize,priority,name,nargs,args);

/*------------------------------------------------------------------------
 *  createProc  -  create a process to start running a procedure
 *------------------------------------------------------------------------
 */
static int createProc(procaddr,ssize,priority,name,nargs,args)
					...
					/* array in the code)		*/
{
  int pid;
	STATWORD 	ps;    
	disable(ps);
  pid = createProc(procaddr, ssize, priority, name, nargs, args);
  if(pid != SYSERR) proctab[pid].rt = FALSE;
	restore(ps);
  return pid;
}

/*syscal to create a realtime process*/
					...
					/* array in the code)		*/
{
  int pid;
	...
	disable(ps);
  pid = createProc(procaddr, ssize, priority, name, nargs, args);
  if(pid != SYSERR) proctab[pid].rt = TRUE;
	restore(ps);
  return pid;
}
